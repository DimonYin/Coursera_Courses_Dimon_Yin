...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    list1 = []
    for dic in statistics:
        computed_statistic = formula(info, dic)
        playerid = dic[info["playerid"]]
        computed_players = (playerid, computed_statistic)
        list1.append(computed_players)

        del computed_players

    lis = []
    for tup in range(0, len(list1)):
        lis.append(list1[tup][1])

    list2 = sorted(lis, reverse=True)

    topplayers = []
    for num in range(0, numplayers):
        topplayers.append(list2[num])

    list3 = []

    for num in topplayers:
        for dic in statistics:
            computed_statistic = formula(info, dic)
            if num == computed_statistic:
                playerid = dic[info["playerid"]]
                computed_players = (playerid, computed_statistic)
                list3.append(computed_players)
                del computed_players
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    list1 = read_csv_as_list_dict(info["masterfile"], info["separator"], info["quote"])
    ...
    for lis in list3:
        for dic in list1:
            if dic[info["playerid"]] == lis[0]:
                firstname = dic[info["firstname"]]
                lastname = dic[info["lastname"]]
                num = '{:.3f}'.format(lis[1])
                strings = str(num)+" --- "+firstname + " " + lastname
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    list1 = read_csv_as_list_dict(info["battingfile"], info["separator"], info["quote"])
    list2 = filter_by_year(list1, year, info["yearid"])
    list3 = top_player_ids(info, list2, formula, numplayers)
    list4 = lookup_player_names(info, list3)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    """
    dic1 = {}
    list1 =[]   # this list of dictionaries only contains fields which we want
    for dic in statistics:
        for field in fields:
            dic1[playerid] = dic[playerid]
            dic1[field] = dic[field]
        list1.append(dic1)
        dic1 = {}

    list2 = []   # this list only contains player without repetition
    for dic in list1:
        if dic[playerid] in list2:
            pass
        else:
            list2.append(dic[playerid])

    dic2 = {}   # build a new dic of dic
    for field in fields:
        for num in list2:
            numbers = 0
            for dic in list1:
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    """
    list1 = read_csv_as_list_dict(info["battingfile"], info["separator"], info["quote"])
    dic2 = aggregate_by_player_id(list1, info["playerid"], info["battingfields"])

    list2 = [] # convert nested dictionaries to list of dictionaries
    for dic in dic2:
        list2.append(dic2[dic])
